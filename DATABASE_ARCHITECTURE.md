...
`20260921000001_add_saved_items_display_name.sql` (§2, §20 row 16). The
current contract for `saved_items` is therefore this snapshot **plus** that
additive successor — the snapshot itself is never regenerated to contain it.
...
3. Then apply the **additive migrations newer than the snapshot** — currently
   ...
   nullable `saved_items.display_name` and reloads the PostgREST cache). Each
   additive migration is self-contained and states its own copy-pasteable
   SQL; the snapshot is never regenerated to include it, so the trio above
