...
LifeOS uses [Supabase](https://supabase.com/) (hosted PostgreSQL) for data persistence. Four tables store saved items, bio state, structured logs, and helper panel configuration.
...
| `panel_settings` | Helper panel configuration — typed columns, single row (`key='global'`) |

All tables have RLS enabled. SELECT is granted to `anon` + `authenticated` (dashboard reads). All writes use the service-role key.

#### `panel_settings` Schema

The `panel_settings` table is the permanent configuration storage for the entire helper panel system. It uses typed columns (not key-value) so every setting has a proper SQL type, a default, and a CHECK constraint. Supabase is the single source of truth — the Settings panel reads every value from the database and writes changes back immediately.

| Column | Type | Default | Purpose |
|---|---|---|---|
| `key` | text (PK) | `'global'` | Singleton row key — always `'global'` |
| `auto_close_enabled` | boolean | `true` | Whether inline panels auto-close after the delay |
| `auto_close_delay_seconds` | integer | `120` | Seconds before an inline panel auto-closes (10–3600) |
| `max_deep_save_mb` | integer | `50` | Maximum file size in MB for deep saves (1–500) |
| `delete_batch_size` | integer | `100` | Messages per `delete_messages()` API call (1–1000) |
| `log_retention_days` | integer | `7` | Days of logs to retain before cleanup (1–365) |
| `panel_countdown_interval` | integer | `30` | Seconds between countdown re-renders (5–120) |
| `input_timeout_seconds` | integer | `120` | Seconds before a pending input request expires (10–600) |
| `updated_at` | timestamptz | `now()` | Last update timestamp |

All integer columns have CHECK constraints enforcing their valid ranges. The table is created by the migration `20260726143924_create_panel_settings_table.sql` and expanded by `expand_panel_settings_typed_columns`. Existing installations migrate automatically — no manual editing required.
