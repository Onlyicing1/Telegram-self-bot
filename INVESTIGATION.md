...
- **`settings_set` unknown-key phantom success (RC-5, FIXED 2026-09-05)**: unknown keys now return `False` / `ToolResult(success=False)` at the service and tool boundaries — no DB attempt, no cache pollution — and are regression-tested in `tests/test_settings_unknown_key.py` (9 tests).
