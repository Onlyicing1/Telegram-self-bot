...
**Source:** `backend/observability/db_stats.py` — reads from `RepositoryManager` repositories.
...
entries that are already expired.
...
| GET | `/api/status` | Runtime status (memory, CPU, workers) |
...
├── __init__.py
├── runtime_status.py    — process + worker metrics
├── ai_stats.py          — AI execution statistics
├── db_stats.py          — database repository statistics
├── health_snapshot.py   — unified snapshot generator
├── crash_report.py      — crash report generator
├── performance.py       — performance report generator
└── maintenance.py       — safe maintenance utilities
```
